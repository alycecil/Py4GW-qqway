...
BOT_NAME = "DoA Gemstone Farm"
TEXTURE = os.path.join(Py4GW.Console.get_projects_path(), "Bots", "Vanquish", "VQ_Helmet.png")

MODULE_NAME = "DoA Gemstone Farm"
MODULE_ICON = "Textures/Skill_Icons/[54] - Crippling Anguish.jpg"

ID_GATE_OF_ANGUISH = 474
ID_THE_EBONY_CITADEL_OF_MALLYX = 445
...
BATTLE_HEADER = f"[H]Walk to High Priest Zhellixs Location_5"
TALK_TO_NPC = f"[H]Talk To Zhellix_2"


def Fight(bot: Botting) -> None:
    global TALK_TO_NPC, BATTLE_HEADER
    #events
    ...
    # TalkToZhellix()
    bot.Map.Travel(target_map_id=ID_GATE_OF_ANGUISH)
    bot.Wait.ForTime(12134)
    bot.Wait.ForMapLoad(target_map_id=ID_GATE_OF_ANGUISH)
    ...
    TALK_TO_NPC = bot.States.AddHeader("Talk To Zhellix")

    bot.Move.XYAndInteractNPC(6081, -13314)
    bot.Wait.ForTime(2134)
    bot.Move.XYAndDialog(6081, -13314, 0x81)
    bot.Wait.ForTime(2456)
    bot.Move.XYAndDialog(6081, -13314, 0x84)
    bot.Wait.UntilOnExplorable()

    bot.States.AddHeader("Waiting before going to npc")
    bot.Wait.ForTime(31324)

    bot.States.AddCustomState(lambda: PrepareForBattle(bot), "PrepareForBattle")

    bot.States.AddHeader("Walk to High Priest Zhellix")
    bot.Move.XYAndInteractNPC(-3198.00, -4928.00)
    ConfigureAggressiveEnv(bot)

    BATTLE_HEADER = bot.States.AddHeader("Walk to High Priest Zhellixs Location")
    bot.Move.XY(-3198.00, -4928.00)
    bot.Party.FlagAllHeroes(-3198.00, -4928.00)
    bot.Wait.ForTime(15000)
    bot.Move.XY(-3198.00, -4928.00)
    bot.Wait.UntilOutOfCombat(Range.Earshot)
    bot.Move.XY(-3198.00, -4928.00)
    bot.Wait.UntilOutOfCombat(Range.Earshot)
    bot.Items.LootItems()
    bot.Move.XY(-3198.00, -4928.00)
    bot.Wait.UntilOutOfCombat(Range.Earshot)
    bot.States.AddCustomState(lambda: _do_we_loop(bot), "Are we done")
    bot.Items.LootItems()
    bot.Move.XY(-3198.00, -4928.00)
    bot.Items.LootItems()
    ...
    bot.States.JumpToStepName(TALK_TO_NPC)
...
def _do_we_loop(bot):
    if not Map.IsMapIDMatch(Map.GetMapID(), ID_THE_EBONY_CITADEL_OF_MALLYX):
        return

    if Agent.IsDead(15):
        return

    npc_x, npc_y = Agent.GetXY(15)

    from Py4GWCoreLib.py4gwcorelib_src.Utils import Utils

    distance = Utils.Distance((-3198.00, -4928.00), (npc_x, npc_y))

    if distance < 1500:
        fsm = bot.config.FSM
        fsm.jump_to_state_by_name(BATTLE_HEADER)
        fsm.resume()
...
def tooltip():
    ...
    PyImGui.text_colored("Mallyx Farm", title_color.to_tuple_normalized())
    ImGui.pop_font()
    PyImGui.spacing()
    PyImGui.separator()
    # Description
    PyImGui.text("Multi Account, farm gemstones from Mallyx ritual")
    PyImGui.spacing()
    # Credits
    PyImGui.text_colored("Credits:", title_color.to_tuple_normalized())
    PyImGui.bullet_text("Developed by some weird internet girl")
    PyImGui.end_tooltip()
...
def _draw_statistics_tab() -> None:
    import PyImGui
    if PyImGui.begin_child("MallyxStatisticsTabChild", _EXPANDED_TAB_CHILD_SIZE, False):
        _draw_title_track()
    PyImGui.end_child()
...
def _draw_heroes_tab() -> None:
    import PyImGui
    if PyImGui.begin_child("MallyxHeroesTabChild", _EXPANDED_TAB_CHILD_SIZE, False):
