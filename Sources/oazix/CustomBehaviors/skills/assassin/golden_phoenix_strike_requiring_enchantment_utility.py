...
class Golden_Phoenix_Strike_RequiringEnchantmentUtility(CustomSkillUtilityBase):
    def __init__(self,
    event_bus: EventBus,
    ...
    allowed_states: list[BehaviorState] = [BehaviorState.IN_AGGRO]
        ...
        super().__init__(
            event_bus=event_bus,
            skill=CustomSkill("Golden_Phoenix_Strike"),
    ...
    def _get_targets(self) -> list[custom_behavior_helpers.SortableAgentData]:
        return custom_behavior_helpers.Targets.get_all_possible_enemies_ordered_by_priority_raw(
            ...
            sort_key=(TargetingOrder.DISTANCE_ASC, TargetingOrder.AGENT_QUANTITY_WITHIN_RANGE_DESC),
            range_to_count_enemies=GLOBAL_CACHE.Skill.Data.GetAoERange(self.custom_skill.skill_id))

    @override
    def _evaluate(self, current_state: BehaviorState, previously_attempted_skills: list[CustomSkill]) -> float | None:

        has_enchantment = Agent.IsEnchanted(Player.GetAgentID())
        if not has_enchantment: return None
    ...
    def customized_debug_ui(self, current_state):
        PyImGui.bullet_text(f"has_enchantment : {Agent.IsEnchanted(Player.GetAgentID())}")