...
class DwaynasSorrowUtility(CustomSkillUtilityBase):
    def __init__(self,
        event_bus: EventBus,
        ...
        super().__init__(
            event_bus=event_bus,
            skill=CustomSkill("Dwaynas_Sorrow"),
    ...
    def _get_target(self) -> int | None:
        get_agent_id = Player.GetAgentID()
        has_buff = Routines.Checks.Effects.HasBuff(get_agent_id, self.custom_skill.skill_id)
        if not has_buff: return get_agent_id
        ...
        minion_array = AgentArray.Filter.ByCondition(minion_array, lambda agent_id: Agent.IsAlive(agent_id) and Agent.GetHealth(agent_id) > 0.05 and not Agent.IsSpirit(agent_id))
