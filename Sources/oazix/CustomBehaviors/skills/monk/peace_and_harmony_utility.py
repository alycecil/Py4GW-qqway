...
class PeaceAndHarmonyUtility(CustomSkillUtilityBase):
    ...
    def __init__(self,
        ...
        score_definition: ScoreStaticDefinition = ScoreStaticDefinition(51),
    ...
    def _get_target_score(self, target: custom_behavior_helpers.SortableAgentData) -> float:
        ...
        if Agent.IsConditioned(target_agent_id) and Agent.IsHexed(target_agent_id):
            score_offset += 3

        if Agent.IsDegenHexed(target_agent_id):
            score_offset += 3
        elif Agent.IsHexed(target_agent_id):
            score_offset += 2
        ...
        score: int = round(self.score_definition.get_score()) + score_offset
