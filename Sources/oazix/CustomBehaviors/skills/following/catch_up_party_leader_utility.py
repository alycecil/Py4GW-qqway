...
from Py4GWCoreLib import GLOBAL_CACHE, Agent, Range, Player, AutoPathing, Routines
...
class CatchUpPartyLeaderUtility(CustomSkillUtilityBase):
    """
    ...
    def __init__(
        ...
        score_static_definition = ScoreStaticDefinition(20)

        super().__init__(
            event_bus=event_bus,
            skill=CustomSkill("catch_up_party_leader_utility"),
            in_game_build=current_build,
            score_definition=score_static_definition,
            allowed_states=[BehaviorState.IN_AGGRO, BehaviorState.CLOSE_TO_AGGRO, BehaviorState.FAR_FROM_AGGRO],
            utility_skill_typology=UtilitySkillTypology.COMBAT)

        self.throttle_timer = ThrottledTimer(1200)
        self.score_definition: ScoreStaticDefinition = score_static_definition
        ...
        self.follow_distance_tolerance = Range.Spellcast.value  # Tolerance for follow distance
    ...
    def _get_follow_distance_for_state(self, state: BehaviorState) -> float:
        ...
        return self.follow_distance_tolerance
    ...
    def _execute(self, state: BehaviorState) -> Generator[Any, None, BehaviorResult]:
        ...
        try:
            ...
            if target_pos is not None:
                ...
                path3d = yield from AutoPathing().get_path_to(target_pos[0], target_pos[1], smooth_by_los=True, margin=400.0, step_dist=242.0)
                ...
                yield from Routines.Yield.Movement.FollowPath(
                    ...
                    log=constants.DEBUG,
                    timeout=5_000,
                    progress_callback=lambda progress: ConsoleLog("CatchUpPartyLeaderUtility", f"FollowPath: progress: {progress}", Console.MessageType.Info) if constants.DEBUG else None,
    ...
    def customized_debug_ui(self, current_state: BehaviorState) -> None:
        ...
        if PyImGui.collapsing_header("Follow Leader Only Status", PyImGui.TreeNodeFlags.DefaultOpen):
            ...
            if self._am_i_party_leader():
                ...
                self.follow_distance_tolerance = PyImGui.slider_float(
                    "Follow Distance",
                    self.follow_distance_tolerance,
                    50.0,
                    3000.0
                )
